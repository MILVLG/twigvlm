...
import torch
import transformers
# from transformers.models.llama.modeling_llama import _prepare_4d_causal_attention_mask, _prepare_4d_causal_attention_mask_for_sdpa, _prepare_4d_attention_mask
...
def crop_past_key_values(
    ...
    exit_vision_layer: Optional[int] = -1, 
    length_after_fastv: Optional[int] = None,  
...
def forward(
    ...
    for layer_id, decoder_layer in enumerate(model.model.layers):
        if layer_id == exit_layer and is_first_forward and enable_FastV:
            hidden_states_pre = hidden_states.clone()
            for idx, ex_decoder_layer in enumerate(model.extra_layers):
                if idx == 2:
...
def forward_early(
    ...
    if cache_length != 0 and forward_earlyp_idx == 0:
    ...
    for layer_id, decoder_layer in enumerate(model.extra_layers):
        if layer_id == 2 and enable_FastV and is_first_forward:
    ...
    hidden_states = model.lm_head_early_norm(hidden_states)

    logits = model.lm_head_early(hidden_states)
